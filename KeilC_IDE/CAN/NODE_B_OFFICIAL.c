...
void init_EPWM(void){
	...
	PORTD->PCR[0] |= PORT_PCR_MUX(2); //alt2: FTM0_CH2, Blue LED
	...
	//chan2
	FTM0->CONTROLS[2].CnSC |=  FTM_CnSC_ELSB_MASK| FTM_CnSC_ELSA_MASK;

	FTM0->CONTROLS[2].CnV = FTM_CnV_VAL(0); // 0%
	...
	FTM0->SC	|= FTM_SC_CLKS(3)| FTM_SC_PS(3) | FTM_SC_PWMEN1_MASK | FTM_SC_PWMEN2_MASK; //choose SOSCDIV1 , prescale: 8 	8MHz : 8 = 1MHz, CHOSE PWM CHANNEL 1
...
void FLEXCAN0_init(void){
		...
		/* IDE=0: Standard ID */
		 ...
		 CAN0->RAMn[ 4*MSG_BUF_SIZE + 1] = 0x02c00000;/* Msg Buf 4, word 1: Standard ID = 0xB0 */
		#else /* Node B to receive msg with std ID 0x555 */
		 //CAN0->RAMn[ 4*MSG_BUF_SIZE + 1] = 0x15540000; /* Msg Buf 4, word 1: Standard ID = 0x555 */
		 CAN0->RAMn[ 4*MSG_BUF_SIZE + 1] =  0x02800000; /* Msg Buf 4, word 1: Standard ID = 0xA0 */
		#endif
...
void FLEXCAN0_transmit_msg (void){ /* Assumption: Message buffer CODE is INACTIVE */
			 ...
			 //CAN0->RAMn[ 0*MSG_BUF_SIZE + 1] = 0x14440000; /* MB0 word 1: Tx msg with STD ID 0x511 */
				CAN0->RAMn[ 0*MSG_BUF_SIZE + 1] = 0x02c00000;/* MB0 word 1: Tx msg with STD ID 0xB0 */
			#endif
		...
		switch (mode)
    {
    	case 1: //CAN is transfering message
			...
			case 2:
				...
				FTM0->CONTROLS[2].CnV = FTM_CnV_VAL(RxDATA[0]); 
				break;
    	default:// stop CAN transmission
				FTM0->CONTROLS[1].CnV = FTM_CnV_VAL(1000); //green LED on after reset
				FTM0->CONTROLS[2].CnV = FTM_CnV_VAL(0); //green LED on after reset
				CAN0->RAMn[ 0*MSG_BUF_SIZE + 0] = 0x08000000; //tell the buffer to go INACTIVE
    		break;
    }
	}
}
