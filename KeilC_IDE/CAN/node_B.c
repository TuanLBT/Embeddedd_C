...
void FLEXCAN0_init(void){
		...
		/* IDE=0: Standard ID */
		 ...
		 CAN0->RAMn[ 2*MSG_BUF_SIZE + 1] = 0x15540000; /* Msg Buf 4, word 1: Standard ID = 0x555 */
		#endif
...
void FLEXCAN0_receive_msg (void){
			 uint8_t j;
			 uint32_t dummy;
			 RxCODE = (CAN0->RAMn[ 2*MSG_BUF_SIZE + 0] & 0x07000000) >> 24; /* Read CODE field */
			 RxID = (CAN0->RAMn[ 2*MSG_BUF_SIZE + 1] & CAN_WMBn_ID_ID_MASK) >>CAN_WMBn_ID_ID_SHIFT ;
			 RxLENGTH =(CAN0->RAMn[ 2*MSG_BUF_SIZE + 0] & CAN_WMBn_CS_DLC_MASK)>>CAN_WMBn_CS_DLC_SHIFT;
			 for (j=0; j<2; j++) { /* Read two words of data (8 bytes) */
			 RxDATA[j] = CAN0->RAMn[ 2*MSG_BUF_SIZE + 2 + j];
			 }
			 RxTIMESTAMP = (CAN0->RAMn[ 0*MSG_BUF_SIZE + 0] & 0x000FFFF);
			 dummy = CAN0->TIMER; /* Read TIMER to unlock message buffers */
			 CAN0->IFLAG1 = CAN_IFLAG1_BUF4TO1I_MASK;//0x00000010; /* Clear CAN 0 MB 4 flag without clearing others*/
			 if(dummy == 0){}
}
...
int main(void) {
 ...
 while(1)  /* Loop: if a msg is received, transmit a msg */
	{ 
	 if ((CAN0->IFLAG1 >> 2) & 1)  /* If CAN 0 MB 4 flag is set (received msg), read MB4 */
		{ 
